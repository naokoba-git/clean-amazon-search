...
### 4. カスタム/非表示ブランド管理
- **カスタム信頼ブランド**: ユーザーが独自の信頼ブランドを追加可能
- **非表示ブランド**: デフォルト信頼ブランドから除外（信頼バッジの×ボタンまたはポップアップから操作）
- **chrome.storage.sync**: Googleアカウント同期で異なるPCでも設定が反映
- **ストレージキー**: `customBrands`（追加）、`excludedBrands`（除外）
...
### v1.3.0 (2026-02-08)
- **非表示ブランド機能追加**: デフォルト信頼ブランドから除外する機能
- **バッジ×ボタン**: 検索結果の信頼バッジに×ボタン追加（クリックで非表示ブランドに追加）
- **chrome.storage.sync移行**: カスタム/非表示ブランドをGoogleアカウント同期対応
- **ポップアップUI**: 非表示ブランド管理セクション追加
- **リファクタリング**: BrandsManagerファクトリ統合、CSS共通クラス化、二重extractProductInfo解消
- **local→syncマイグレーション**: 既存ユーザーのデータ自動移行
...
- [x] セラーのブラックリスト/ホワイトリスト機能 → 非表示ブランド/カスタム信頼ブランドで実現
- [ ] 判定結果のキャッシュ改善
- [ ] タイトル長判定の閾値調整
- [x] chrome.storage.sync対応（Googleアカウント同期）
